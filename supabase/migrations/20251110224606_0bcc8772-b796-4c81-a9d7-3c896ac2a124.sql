-- Corrigir função generate_contract_installments para validar bank_account_id antes de gerar transações
...
BEGIN
  FOR contract_record IN 
    ...
    WHERE is_active = true 
      ...
      AND bank_account_id IS NOT NULL  -- NOVO: Apenas contratos com conta bancária definida
  LOOP
    parcelas_count := 0;
    iteration_count := 0;
    
    -- Mapear tipo do contrato para tipo de transação
    transaction_type := CASE 
      WHEN contract_record.type = 'income' THEN 'revenue'
      WHEN contract_record.type = 'revenue' THEN 'revenue'
      WHEN contract_record.type = 'expense' THEN 'expense'
      WHEN contract_record.type = 'transfer' THEN 'transfer'
      ELSE 'revenue'
    END;
    ...
    WHILE current_date_calc <= CURRENT_DATE AND iteration_count < max_iterations LOOP
      ...
      -- Inserir a transação com tipo correto
      INSERT INTO public.transactions (
        ...
        CASE WHEN transaction_type = 'expense' THEN contract_record.bank_account_id END,
        CASE WHEN transaction_type = 'revenue' THEN contract_record.bank_account_id END,
